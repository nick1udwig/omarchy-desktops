...
`Overview.qml` coordinates one `OverviewScreen.qml` surface per Quickshell screen. The opening state and filter are shared, while each surface owns its window selection and Quick Look state. Only one surface requests exclusive keyboard focus; clicking or hovering another monitor's cards, clicking its filter, or pressing Tab transfers keyboard control. Screen removal drops the corresponding surface and chooses another keyboard target.
